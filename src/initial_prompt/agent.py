...
class Agent:
    ...
    def make_decision(self, prompt: str, project_name: str) -> str:
        ...
        for item in decision:
            ...
            self.project_manager.add_message_from_angelic(project_name, reply)
            ...
            if function == "git_clone":
                ...
                self.project_manager.add_message_from_angelic(project_name, response)
    ...
    def execute(self, prompt: str, project_name_from_user: str = None) -> str:
        ...
        self.project_manager.add_message_from_angelic(project_name, reply)
        self.project_manager.add_message_from_angelic(project_name, json.dumps(plans, indent=4))
        self.project_manager.add_message_from_angelic(project_name, f"In summary: {summary}")
        ...
        if (queries and len(queries) > 0) or ask_user != "":
            self.project_manager.add_message_from_angelic(
                project_name,
        ...
        if not queries and len(queries) == 0:
            self.project_manager.add_message_from_angelic(project_name,
        ...
        if ask_user != "" and ask_user is not None:
            self.project_manager.add_message_from_angelic(project_name, ask_user)
            self.agent_state.set_agent_active(project_name, False)
            ...
            while not got_user_query:
                ...
                if latest_message_from_user and validate_last_message_is_from_user:
                    ...
                    got_user_query = True
                    self.project_manager.add_message_from_angelic(project_name, "Thanks! 🙌")
                time.sleep(5)
